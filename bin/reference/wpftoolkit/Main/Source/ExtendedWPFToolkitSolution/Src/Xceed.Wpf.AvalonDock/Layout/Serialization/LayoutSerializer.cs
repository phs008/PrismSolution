...
namespace Xceed.Wpf.AvalonDock.Layout.Serialization
{
    public abstract class LayoutSerializer
    {
        ...
        protected virtual void FixupLayout(LayoutRoot layout)
        {
            ...
            //var tempFix = layout.Descendents().OfType<LayoutAnchorable>();
            //var exlucdeIDs = new HashSet<string>(tempFix.Select(x => x.ContentId));
            //// TODO : Layout 에 추가하는 접근 방법을 사용자 편의성을 고려해 좀더 고민해야함.
            //var targetList = _previousAnchorables.ToArray().Where(x => !exlucdeIDs.Contains(x.ContentId)).ToList();
            //if (targetList.Count > 0)
            //{
            //    foreach (var lcToFix in targetList)
            //    {
            //        (tempFix.ElementAt(0).Parent as LayoutAnchorablePane).Children.Add(lcToFix);
            //    }
            //}
