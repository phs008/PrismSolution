...
namespace Xceed.Wpf.Toolkit
{
  ...
  [TemplatePart( Name = PART_EyeDropperBtn , Type = typeof(Button))]
  public class ColorCanvas : Control
  {
    ...
    private const string PART_EyeDropperBtn = "PART_EyeDropperBtn";
    ...
    private bool _IsEyeDropperStarted = false;
    private static int Win32idHook;
    private int strid;
    private byte[] pixels;
    private Int32Rect rect;
    private Win32Helper.HookProc Win32HookProc;
    ...
    public static readonly DependencyProperty EyeDropperProperty = DependencyProperty.Register("EyeDropper", typeof(Button), typeof(ColorCanvas), new PropertyMetadata(null));

    public Button EyeDropper
    {
        get { return (Button)GetValue(EyeDropperProperty); }
        set { SetValue(EyeDropperProperty, value); }
    }
    ...
    public override void OnApplyTemplate()
    {
      ...
      if (_eyeDropperBtn != null)
        _eyeDropperBtn.PreviewMouseLeftButtonDown -= new MouseButtonEventHandler(EyeDropper_PreviewMouseBtnDown);
      
        _eyeDropperBtn = GetTemplateChild(PART_EyeDropperBtn) as Button;

      if (_eyeDropperBtn != null)
        _eyeDropperBtn.PreviewMouseLeftButtonDown += new MouseButtonEventHandler(EyeDropper_PreviewMouseBtnDown);

      Win32HookProc = new Win32Helper.HookProc(Win32EventHookProc);
    ...
    void EyeDropper_PreviewMouseBtnDown(object sender, MouseButtonEventArgs e )
    {
      _IsEyeDropperStarted = true;
      Win32idHook = Win32Helper.SetWindowsHookEx(14, Win32HookProc, (IntPtr)0, 0);
    }
    [SecuritySafeCritical()]
    int Win32EventHookProc(int nCode,IntPtr wParam,IntPtr lParam)
    {
      MouseHookStruct mousePoint = (MouseHookStruct)Marshal.PtrToStructure(lParam, typeof(MouseHookStruct));
      MouseMessages message = (MouseMessages)wParam;
      if (message == MouseMessages.WM_LBUTTONUP)
        Win32Helper.UnhookWindowsHookEx(Win32idHook);
      else
      {
        BitmapSource source = Win32Helper.CaptureRegion(Win32Helper.GetDesktopWindow(),
        ...
        if(source != null)
        {
          strid = (source.PixelWidth * source.Format.BitsPerPixel + 7) / 8;
          pixels = new byte[source.PixelHeight * strid];
          rect = new Int32Rect(mousePoint.pt.x, mousePoint.pt.y, 2, 2);
          source.CopyPixels(rect, pixels, strid, 0);
          R = pixels[2];
          G = pixels[1];
          B = pixels[0];
        }
