...
namespace VRCAT.WrapperBridge
{
    ...
    public class RenderingViewHost : HwndHost
    {
        ...
        public RenderingViewHost(int width, int height)
        {
        ...
        public void HandleMouseDown(object sender, MouseButtonEventArgs e)
        {
            ...
            //{
            //    MContainer pickObject = _3DWindow.SelectPolygonPickObject((int)p.X, (int)p.Y, _RenderingCamera);
            //    if(pickObject != null)
            //    {
        ...
        public void HandleKeyDown(object sender, KeyEventArgs e)
        {
            if(Mouse.RightButton == MouseButtonState.Pressed)
            {
                if (e.Key == Key.W)
                {
                    _RenderingCamera.Transform.MoveForward(5);
                }
                else if (e.Key == Key.A)
                {
                    _RenderingCamera.Transform.Move(-5, 0, 1);
                }
                else if(e.Key == Key.D)
                {
                    _RenderingCamera.Transform.Move(5, 0, 1);
